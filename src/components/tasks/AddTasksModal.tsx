            ...
            >
                <DialogPanel className="max-h-full overflow-y-scroll rounded-3xl w-full sm:w-xl max-w-4xl space-y-4 bg-(--content) p-5 sm:p-8 sm:pb-8">
                    <div className='flex justify-between items-center mb-3 mt-3'>
                        <DialogTitle className="font-bold text-4xl ">Create task</DialogTitle>
                        <XIcon onClick={closeModal} size={23} className='cursor-pointer' />
                    </div>

                    <p className='text-sm text-(--white)/50 mb-8'>
                        Create a new task by entering the necessary details such as name and description. Stay organized and keep track of your to-dos efficiently.
                    </p>
                    ...
                    <form onSubmit={handleSubmit(handleForm)} noValidate>
                        ...
                        <div className="flex justify-end gap-2">
                            ...
                            <input type="submit" disabled={ status === 'pending' } value="Create Task" className="bg-(--white) hover:bg-(--white)/80 cursor-pointer py-2 px-5 text-nowrap text-md text-(--black) rounded-lg" />
                        </div>
                    </form>
